"""Phase 2F — α sweep with iterative manifold transport (in-k).

Mirrors `p2e_alpha_sweep.py` but adds an `iterative_manifold` mode in addition
to baseline / chord-manifold (Phase 2E) / primal. The iterative delta is
computed by `p2f_iterative_manifold.iterative_manifold_delta`; everything else
(model load, hook, evaluation, prompt selection, primal direction) reuses the
unmodified Phase 2E machinery via import.

Output JSON:
  results/p2f_iterative_alpha_sweep_<model>_<pair>_k<k>.json

Schema:
  {
    "model": ..., "pair": ..., "k": ..., "layer": ...,
    "iterative_meta": { ...summary stats from IterativeMeta... },
    "delta_diagnostics": {
        "mean_chord_norm": ..., "mean_iter_norm": ...,
        "mean_diff_to_chord_norm": ..., "frac_diff_above_1e-6": ...,
    },
    "results": {
        "baseline":           {"alpha": null, ...},
        "manifold_a0.00":     {"alpha": 0.0, ...},   # chord (Phase 2E)
        "iterative_a0.00":    {"alpha": 0.0, ...},   # path-summed (Phase 2F)
        "primal_a0.00":       {"alpha": 0.0, ...},
        ...
    }
  }

Each per-α entry includes r_ld_zeff, r_ld_x, mean_ld, std_ld, n.

Usage:
  python3 scripts/p2f_iterative_alpha_sweep.py --model gemma2-2b --pair height --k 15
"""
...
def main():
    ...
    p.add_argument("--skip-chord", action="store_true",
                    help="skip the Phase 2E chord-manifold mode (default: run it for "
                         "side-by-side comparison)")
    p.add_argument("--skip-primal", action="store_true",
                    help="skip primal proj_out mode")
    args = p.parse_args()
    bs = args.batch_size or (32 if args.model == "gemma2-2b" else 8)

    print(f"[2F αsweep] loading {MODEL_ID[args.model]}...", flush=True)
    ...
    print(f"[2F αsweep] words: low={low_word!r}({low_id}) "
          f"high={high_word!r}({high_id})", flush=True)
    ...
    # Hold-out split (cell_seed != 0), same as Phase 2E.
    test_mask = cs_arr != 0
    ...
    print(f"[2F αsweep] {args.model}/{args.pair} k={args.k}: n={n_test} "
          f"max_seq={max_seq}", flush=True)
    print(f"[2F αsweep] full-grid iterative meta: {meta.summary()}", flush=True)
    print(f"[2F αsweep] test-set delta diagnostics: {delta_diag}", flush=True)
    ...
    out_path = (REPO / "results"
                / f"p2f_iterative_alpha_sweep_{args.model}_{args.pair}_k{args.k}.json")
